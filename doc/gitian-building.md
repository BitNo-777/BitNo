...
*Setup instructions for a gitian build of bitno using a VM or physical system.*

Gitian is the deterministic build process that is used to build the bitno
...
to the bitno GitHub release page.
...
- [Building bitno](#building-bitno-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/bitno-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/bitno-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/bitno-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/bitno-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/bitno-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/bitno-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/bitno-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@www.bitno.casino) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/bitno-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/bitno-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/bitno-osx-*-build.assert
```
