...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("bitno CoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        return error("bitno CoinMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("bitno CoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("bitno-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running bitno CoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("bitno CoinMiner exception");
    } catch (...) {
        LogPrintf("bitno CoinMiner exception");
    }

    LogPrintf("bitno CoinMiner exiting\n");
}
