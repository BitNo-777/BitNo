...
bool CBITNOStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CBITNOStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CBITNOStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CBITNOStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CBITNOStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CBITNOStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CBITNOStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CBITNOStake::GetUniqueness() const
{
    //The unique identifier for a BITNO stake is the outpoint
...
CBlockIndex* CBITNOStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CBITNOStake::ContextCheck(int nHeight, uint32_t nTime)
{
