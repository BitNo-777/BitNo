...
#include "qt/bitno/forms/ui_masternodeswidget.h"

#include "qt/bitno/qtutils.h"
#include "qt/bitno/mnrow.h"
#include "qt/bitno/mninfodialog.h"
#include "qt/bitno/masternodewizarddialog.h"
...
#include "util.h"
#include "qt/bitno/optionbutton.h"
#include <iostream>
#include <fstream>
...
MasterNodesWidget::MasterNodesWidget(bitnoGUI *parent) :
    PWidget(parent),
    ...
    if (fs::exists(pathBootstrap)) {
        ...
        if (lineCopy.size() == 0) {
            lineCopy = "# Masternode config file\n"
                                    ...
                                    "# Example: mn1 127.0.0.2:53330 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0\n";
        }
...
void MasterNodesWidget::onCreateMNClicked()
{
    ...
    if (walletModel->getBalance() <= GetMNCollateral(chainActive.Height()) * COIN) {
        inform(tr("Not enough balance to create a masternode, %1 BITNO required.").arg(GetMNCollateral(chainActive.Height())));
        return;
    }
    showHideOp(true);
