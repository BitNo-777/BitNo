...
#include "qt/bitno/forms/ui_snackbar.h"
#include "qt/bitno/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(bitnoGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &bitnoGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
