...
#include "qt/bitno/forms/ui_send.h"
#include "qt/bitno/addnewcontactdialog.h"
#include "qt/bitno/qtutils.h"
#include "qt/bitno/sendchangeaddressdialog.h"
#include "qt/bitno/optionbutton.h"
#include "qt/bitno/sendconfirmdialog.h"
#include "qt/bitno/myaddressrow.h"
#include "qt/bitno/guitransactionsutils.h"
...
SendWidget::SendWidget(bitnoGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-bitno");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination bitnoAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(bitnoAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(bitnoAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
