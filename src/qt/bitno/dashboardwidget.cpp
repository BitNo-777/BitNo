...
#include "qt/bitno/dashboardwidget.h"
#include "qt/bitno/forms/ui_dashboardwidget.h"
#include "qt/bitno/sendconfirmdialog.h"
#include "qt/bitno/txrow.h"
...
DashboardWidget::DashboardWidget(bitnoGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareBITNO, "square-chart-bitno");
    setCssProperty(ui->labelBITNO, "text-chart-bitno");
    ...
    setCssProperty(ui->labelAmountBITNO, "text-stake-bitno-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &bitnoGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &bitnoGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &bitnoGUI::openFAQ);
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            bitno = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalBITNO += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesBITNO.append(bitno);

        int max = std::max(bitno, bitno);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesBITNO);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalBITNO > 0) {
        setCssProperty(ui->labelAmountBITNO, "text-stake-bitno");
    } else {
        setCssProperty(ui->labelAmountBITNO, "text-stake-bitno-disable");
    }
    forceUpdateStyle({ui->labelAmountBITNO});
    ui->labelAmountBITNO->setText(GUIUtil::formatBalance(chartData->totalBITNO, nDisplayUnit));
