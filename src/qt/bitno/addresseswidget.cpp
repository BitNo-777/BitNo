...
#include "qt/bitno/forms/ui_addresseswidget.h"
#include "qt/bitno/addresslabelrow.h"
#include "qt/bitno/addnewaddressdialog.h"
#include "qt/bitno/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(bitnoGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination bitnoAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);
        ...
        if (walletModel->updateAddressBookLabels(bitnoAdd, label.toUtf8().constData(),
