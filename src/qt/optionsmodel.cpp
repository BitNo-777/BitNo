...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::BITNO);
    ...
    if (settings.contains("nAnonymizebitnoAmount") || reset)
        SoftSetArg("-anonymizebitnoamount", settings.value("nAnonymizebitnoAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bitno.cpp during shotdown to also remove the window positions
