...
#if defined(NDEBUG)
#error "bitno cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ubitno) are considered zero fee (for relaying, mining and transaction creation)
...
bool RecalculateBITNOSupply(int nHeightStart)
{
	AssertLockHeld(cs_main);
	...
	uiInterface.ShowProgress(_("Recalculating BITNO supply..."), 0);
	while (true) {
		if (pindex->nHeight % 1000 == 0) {
			LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
			int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
			uiInterface.ShowProgress(_("Recalculating BITNO supply..."), percent);
		}
...
{
    util::ThreadRename("bitno-scriptch");
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> bitnoInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                bitnoInputs.push_back(stakeIn);
        }
        const bool hasBITNOInputs = !bitnoInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasBITNOInputs) {
                    ...
                    for (const CTxIn& bitnoIn : bitnoInputs)
                        if(bitnoIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasBITNOInputs) {
                            for (const CTxIn& stakeIn : bitnoInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // bitno: We use certain sporks during IBD, so check to see if they are
